...
//		final VideoSvcApi svc = VideoSvc.init(server, user, pass);
		...
		CallableTask.invoke(new Callable<Collection<dataRecord>>() {

			@Override
			public Collection<dataRecord> call() throws Exception {
			//	return svc.getVideoList();
				return null;
			}
		}, new TaskCallback<Collection<dataRecord>>() {

			@Override
			public void success(Collection<dataRecord> result) {
